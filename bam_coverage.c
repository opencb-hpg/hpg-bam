...
void bam_coverage_counter_print(bam_chromosome_coverage_t* bam_chromosome_coverage_p, char* output_directory, char* input_filename) {  
  ...
  for (int k = 0; k < NUM_OF_CHROMOSOMES; k++) {    
	...
	str_chromosome = str_chromosomes[k]; //num_chromosome_to_string_(chromosome);
	strlen_chromosome = strlen_chromosomes[k]; //strlen(str_chromosome);
	...
	for (int j = 0; j < NTS_PER_COUNTER; j++) {
	  ...
	  if (coverage_counter_p[j] > 0) {
	    ...
	    memcpy(p, str_chromosome, strlen_chromosome); p += strlen_chromosome;
	    memcpy(p, "\t", 1); p += 1;
	    memcpy(p, str_coordinate, strlen_coordinate); p += strlen_coordinate;
	    memcpy(p, "\t", 1); p += 1;    
	    memcpy(p, str_coverage_matrix[coverage_counter_p[j]], strlen_coverage_matrix[coverage_counter_p[j]]); p += strlen_coverage_matrix[coverage_counter_p[j]];    
	    memcpy(p, "\n", 1); p += 1;
