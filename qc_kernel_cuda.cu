...
void call_kernel_basic_stats(dim3 dimGrid, dim3 dimBlock, bam_data_core_t* d_core_data_p, qc_info_t* d_qc_info_t, int* d_strand_counter_p, int* d_map_quality_p,  int* d_alignment_length_p, int num_alignments) {
  kernel_bam_qc_basic_stats<<<dimGrid, dimBlock>>>(d_core_data_p, d_qc_info_t, d_strand_counter_p, d_map_quality_p, d_alignment_length_p, num_alignments);
}
...
__global__ void kernel_bam_qc_map_errors(bam_data_core_t* d_core_data_p, uint32_t* d_cigar_data_p, qc_alignment_t* d_qc_alignment_p, int num_alignments) {
  ...
  if (k < num_alignments) {
    ...
    for (int i=cigar_start_pos; i<cigar_end_pos; i++) {
 	...
 	case BAM_CMATCH: d_qc_alignment_p[k].counters[M] += cigar_num; break;		//M
	case BAM_CINS: d_qc_alignment_p[k].counters[I] += cigar_num; break;		//I
	case BAM_CDEL: d_qc_alignment_p[k].counters[D] += cigar_num; break;		//D
	case BAM_CEQUAL: d_qc_alignment_p[k].counters[EQUAL] += cigar_num; break;	//=
	case BAM_CDIFF: d_qc_alignment_p[k].counters[X] += cigar_num; break;		//X
       }     
    }
